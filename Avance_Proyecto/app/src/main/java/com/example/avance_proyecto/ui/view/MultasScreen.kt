...
fun MultasScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            //ButtonSectionMultas(listOf("Distribución","Tendencia","Evolución"))

            //val datos = estadoSolicitudData
            val multas = MultasDefaultDataSource.itemCardMultas
    ...
    Surface(
        ...
        TextField(
            ...
            keyboardActions = KeyboardActions(
                onSearch = {
                    showToastMultas(context, "Buscando la palabra: $text")
    ...
    Column{
        ...
        LazyRow {
            items(botones.size) {
                Button(
                    onClick = {
                        showToastMultas(context, "Hola $it")
...
@Composable
fun MultasCard(topic: MultasCard, navController: NavController, modifier: Modifier = Modifier) {
...
@Composable
fun MultasData(estadoSolicitudItems: ConteoEstadoSolicitudItem, navController: NavController){
    val descripcion = estadoSolicitudItems.descripcion
    var dataCard : MultasCard
    when(descripcion){
        "Pendiente" -> {
            MultasCard(topic = MultasCard(
...
fun MultasCuadricula(
    ...
    LazyVerticalGrid(
        ...
        if(isError){
            items(topicList) {
                MultasData(it.conteoSolicitudItems, navController)
            }
        }else{
            items(listaConteoEstadoSolicitud) {
                //println("Hola "+it.descripcion)
                MultasData(it, navController)
            }
        }
...
fun showToastMultas(context: Context, message: String) {
